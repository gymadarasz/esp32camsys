...
#define RECORDER_STATE_DEFAULT { false, false }
...
void camera_loop(void * pvParameters) {
    ...
    while(1) {
        ...
        //     esp_camera_fb_return(fb);
        // } else {
        if (!camera_state.recording && !camera_state.streaming) {
            PRINT("CAMERA IDLE - waiting for record request...");
            vTaskDelay(1000 / portTICK_RATE_MS);
        }
        // }
    }
...
void app_main()
{
    ...
    if (motion_button_state) 
        ...
        app_main_start(
            httpd_server, nvs_handle, 500, camera_additional_join_post_data, 
            camera_join_response_handler, camera_setup, camera_loop
        );
    else 
