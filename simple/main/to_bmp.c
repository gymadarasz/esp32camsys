...
//input buffer
static uint32_t _jpg_read_bmp(void * arg, size_t index, uint8_t *buf, size_t len)
{
    rgb_jpg_decoder * jpeg = (rgb_jpg_decoder *)arg;
    if(buf) {
...
static bool jpg2rgb888(const uint8_t *src, size_t src_len, uint8_t * out, jpg_scale_t scale)
{
    ...
    if(esp_jpg_decode(src_len, scale, _jpg_read_bmp, _rgb_write, (void*)&jpeg) != ESP_OK){
        return false;
    }
    return true;
}
...
bool jpg2bmp(const uint8_t *src, size_t src_len, uint8_t ** out, size_t * out_len)
{
    ...
    if(esp_jpg_decode(src_len, JPG_SCALE_NONE, _jpg_read_bmp, _rgb_write, (void*)&jpeg) != ESP_OK){
        return false;
    }
