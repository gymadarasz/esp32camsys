...
esp_err_t http_404_error_handler(httpd_req_t *req, httpd_err_code_t err)
{
    if (strcmp("/hello", req->uri) == 0) {
        ...
        return ESP_FAIL;
    } else if (strcmp("/camera", req->uri) == 0) {
        httpd_resp_send_err(req, HTTPD_404_NOT_FOUND, "/camera URI is not available");
        /* Return ESP_FAIL to close underlying socket */
        return ESP_FAIL;
    }
...
static esp_err_t ctrl_put_handler(httpd_req_t *req)
{
    ...
    if (buf == '0') {
        ...
        ESP_LOGI(TAG, "Unregistering /hello and /echo and /camera URIs");
        httpd_unregister_uri(req->handle, "/hello");
        httpd_unregister_uri(req->handle, "/echo");
        httpd_unregister_uri(req->handle, "/camera");
        /* Register the custom error handler */
        httpd_register_err_handler(req->handle, HTTPD_404_NOT_FOUND, http_404_error_handler);
    }
    else {
        ESP_LOGI(TAG, "Registering /hello and /echo and /camera URIs");
        httpd_register_uri_handler(req->handle, &hello);
        httpd_register_uri_handler(req->handle, &echo);
        httpd_register_uri_handler(req->handle, &camera);
...
esp_err_t motion_detect_camera_init() {
    esp_err_t err = esp_camera_init(&camera_config);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Camera Init Failed");
    }
...
esp_err_t motion_detect_capture() {
...
esp_err_t motion_detect_start() {
    //initialize the camera
    esp_err_t err = motion_detect_camera_init();
    return err;
}

esp_err_t motion_detect_check() {
    esp_err_t res = ESP_OK;
    int64_t fr_start = esp_timer_get_time();
    
    esp_err_t err = motion_detect_capture();
